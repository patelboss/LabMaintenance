...
title Lab Maintenance – Production (Ctrl+C Feature Test)
...
:: ==================================================
:: WARM-UP LOOP (CTRL+C TEST)
:: ==================================================
set REMAIN=%WARMUP_SECONDS%
color 0B
echo [STEP] Entering warmup loop>>"%LOGFILE%"
echo [FEATURE: CTRL+C] Press Ctrl+C to test abort>>"%LOGFILE%"

:COUNTDOWN
if !REMAIN! LEQ 0 goto FINISH

powershell -NoProfile -Command "Start-Sleep -Seconds 1" || goto ABORT
set /a REMAIN-=1
goto COUNTDOWN
...
:ABORT
color 07
echo [FEATURE: CTRL+C] Abort detected>>"%LOGFILE%"
echo [STEP] Stopping CPU load due to abort>>"%LOGFILE%"
...
echo [INFO] Script aborted by user>>"%LOGFILE%"
