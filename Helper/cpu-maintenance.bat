...
:: [5] LOG SETUP (CSV)
:: ==================================================
set "LOGFILE=C:\Lab_Maintenance\maintenance_log.csv"

if not exist "%LOGFILE%" (
    echo Timestamp,PCID,RemainingSeconds,Workers > "%LOGFILE%"
)
set "LIVEFILE=C:\Lab_Maintenance\maintenance_live.csv"
...
:: ==================================================
:COOLDOWN
if exist "%SIGNAL%" del /f /q "%SIGNAL%"
set "CD=600"
color 0A
...
shutdown /s /f /t 60 /c "Maintenance Complete. will be SHUTDOWN after 1 minutes"

choice /c cn /t 50 /d n /n >nul 2>&1
if !errorlevel! equ 1 (
    shutdown /a >nul 2>&1
    cls
    color 0E
